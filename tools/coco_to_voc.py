...
src_base = os.path.join("/data2/competion/tian_dianwang/PaddleDetection/dataset", "coco", "annotations")
dst_base = os.path.join("/data2/competion/tian_dianwang/PaddleDetection/dataset", "VOCdevkitCOCO", "VOCCOCO")
...
sets = {
    "trainval": os.path.join(src_base, "instances_train.json"),
    "test": os.path.join(src_base, "instances_val.json"),
}
...
for stage, filename in sets.items():
    ...
    for k, im in tqdm(images.items(), "Write Annotations"):
        ...
        #"{}.xml".format(str(k).zfill(12))
        name_xml = im['annotation']['filename'].replace('.jpg','.xml').replace('.JPG','.xml').replace('.png','.xml').replace('.PNG','.xml')
        unparse(im,
                open(os.path.join(dst_dirs["Annotations"],  name_xml ), "w"),
